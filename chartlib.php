    ...
    if ($action == 'csv') {
        ...
        if ($allquest == "1") {
            $rowheaders = array('date','partner', 'portfolio','teacher', 'course', 'question1', 'response1', 'question2', 'response2',
                                ...
                                'question7', 'response7', 'question8', 'response9','question9', 'response10', 'question11', 'response11',
                                'question12', 'response12','question13', 'response13','question14', 'response14','question15', 'response15',
                                'question16', 'response16','question17', 'response17');
        }
    }
    $output = array();
    $content = [];
               ...
               if ($allquest == "1") {
                   ...
                   // Get all the questions.
                     ...
                     $sql2 = "SELECT qt.id qtid, qq.id, qq.surveyid, qr.userid, qt.response, qr.submitted, qs.courseid, qq.content, qq.name
                              ...
                              JOIN {questionnaire_response} qr ON qr.id = qt.response_id
                              JOIN {questionnaire_survey} qs ON qs.id = qq.surveyid";
                  $sql3 = "select qr.id, rankvalue response , qu.course courseid, content, q.submitted
                               from {questionnaire_quest_choice} qc 
                               join {questionnaire_response_rank} qr on qr.question_id = qc.question_id 
                               join {questionnaire_response} q on q.id = qr.response_id and qc.id = qr.choice_id 
                               JOIN {questionnaire} qu on qu.id = q.questionnaireid
                   ...
                   $r6 =  str_replace("&nbsp;", '', trim(strip_tags($r6)));
                   
                   $sqlnew = $sql3. $where7;
                   $res7 = $DB->get_records_sql($sqlnew, $params);
                   foreach($res7 as $res) {
                      $r7 = $res->response;
                   }
                   $r7 =  str_replace("&nbsp;", '', trim(strip_tags($r7)));

                   $sqlnew = $sql3. $where8;

                   $res8 = $DB->get_records_sql($sqlnew, $params);
                   foreach($res8 as $res) {
                      $r8 = $res->response;
                   }
                   $r8 =  str_replace("&nbsp;", '', trim(strip_tags($r8)));

                   $sqlnew = $sql3. $where9;
                   $res9 = $DB->get_records_sql($sqlnew, $params);
                   foreach($res9 as $res) {
                      $r9 = $res->response;
                   }
                   $r9 =  str_replace("&nbsp;", '', trim(strip_tags($r9)));

                   $sqlnew = $sql3. $where10;
                   $res10 = $DB->get_records_sql($sqlnew, $params);
                   foreach($res10 as $res) {
                      $r10 = $res->response;
                   }
                   $r10 =  str_replace("&nbsp;", '', trim(strip_tags($r10)));

                   $sqlnew = $sql3. $where11;
                   $res11 = $DB->get_records_sql($sqlnew, $params);
                   foreach($res11 as $res) {
                      $r11 = $res->response;
                   }
                   $r11 =  str_replace("&nbsp;", '', trim(strip_tags($r11)));

                   $sqlnew = $sql3. $where12;
                   $res12 = $DB->get_records_sql($sqlnew, $params);
                   foreach($res12 as $res) {
                      $r12 = $res->response;
                   }
                   $r12 =  str_replace("&nbsp;", '', trim(strip_tags($r12)));

                   $sqlnew = $sql3. $where13;
                   $res13 = $DB->get_records_sql($sqlnew, $params);
                   foreach($res13 as $res) {
                      $r13 = $res->response;
                   }
                   ...
                   $res16 = $DB->get_records_sql($sqlnew, array());
                   foreach($res7 as $res) {
                      $r16 = $res->response;
                   }
     ...
     if ($portfolio > 0) {    
        ...
        switch($nquestionid) {
           ...
           case "7":
              ...
              break;
           case "10":
              $qname = "I am satisfied with the overall quality of this course";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, satisfied response ";              
              break;
           case "11" :
              $qname = "The topics for this course were relevant for my role.";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, topics response ";              
              break;
           case "12" :    
              $qname = "The independent online work activities were well-designed to help me meet the learning targets.";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, online response ";              
              break;
           case "13":
              $qname = "The Zoom meeting activities were well-designed to help me meet the learning targets.";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, zoom response ";              
              break;
           case "14":
              $qname = "I felt a sense of community with the other participants in this course even though we were meeting virtually.";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, community response ";              
              break;
           case "15":
              $qname = "This course helped me navigate remote and/or hybrid learning during COVID-19";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, covid response ";              
              break;
           case "16":
              $qname = "I will apply my learning from this course to my practice in the next 4-6 weeks";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, practice response ";              
              break;
           case "17":
              $qname = "Recommend this course to a colleague or friend";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, reccomend response ";              
              break;
          case "18":
              $qname = "He/she/they facilitated the content clearly.";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, content1 response ";              
              break;
          case "19":
              $qname = "He/she/they effectively built a community of learners.";
              $sql = " SELECT uidsurvey, coursedate, courseid, district, port1name, port2name, teacher1id, teacher2id, community1 response ";              
              break;
          ...
          // exit();
           ...
           if ($display) {
               ...
               if ($allquest == 1) {
                  ...
                  $cr8 = $result->satisfied;
                  $cr8 =  str_replace("&nbsp;", '', trim(strip_tags($cr8)));
                  $cr9 = $result->topics;
                  $cr9 =  str_replace("&nbsp;", '', trim(strip_tags($cr9)));
                  $cr10 = $result->online;
                  $cr10 =  str_replace("&nbsp;", '', trim(strip_tags($cr10)));
                  $cr11 = $result->zoom;
                  $cr11 =  str_replace("&nbsp;", '', trim(strip_tags($cr11)));
                  $cr12 = $result->community;
                  $cr12 =  str_replace("&nbsp;", '', trim(strip_tags($cr12)));
                  $cr13 = $result->covid;
                  $cr13 =  str_replace("&nbsp;", '', trim(strip_tags($cr13)));
                  $cr14 = $result->practice;
                  $cr14 =  str_replace("&nbsp;", '', trim(strip_tags($cr14)));
                  $cr15= $result->reccomend;
                  $cr15 =  str_replace("&nbsp;", '', trim(strip_tags($cr15)));
                  $cr16 = $result->content1;
                  $cr16 =  str_replace("&nbsp;", '', trim(strip_tags($cr16)));
                  $cr17 = $result->community1;
                  $cr17 =  str_replace("&nbsp;", '', trim(strip_tags($cr17)));
                  ...
                  $q3 = "Overall, what went well in this course?";
               	...
               	$q8 = "I am satisfied with the overall quality of this course";
               	$q9 = "The topics for this course were relevant for my role.";
               	$q10 = "The independent online work activities were well-designed to help me meet the learning targets.";
               	$q11 = "The Zoom meeting activities were well-designed to help me meet the learning targets.";
               	$q12 = "I felt a sense of community with the other participants in this course even though we were meeting virtually.";
               	$q13 = "This course helped me navigate remote and/or hybrid learning during COVID-19.";
               	$q14 = "I will apply my learning from this course to my practice in the next 4-6 weeks.";
               	$q15 = "Recommend this course to a colleague or friend.";
               	$q16 = "He/she/they facilitated the content clearly.";
               	$q17 = "He/she/they effectively built a community of learners";


                  $output[] = array($sub, $partnerdisplay, $portdisplay, $tname, $cfname, $qname, $cr1, $q2, $cr2, $q3,$cr3,
                                      $q4, $cr4, $q5, $cr5, $q6, $cr6, $q7, $cr7, $q8, $cr8, $q9, $cr9, $q10, $cr10, $q11, $cr11,
                                      $q12, $cr12, $q13, $cr14, $q15, $cr15, $q16, $cr16, $q17, $cr17 );
